...
struct DiagnoseView: View {
    ...
    @AppStorage("wfd.lastDiagURL") private var lastDiagURL = "https://www.apple.com"
    @AppStorage("wfd.recentDiagURLs") private var recentDiagURLs = ""
            ...
            VStack(spacing: 8) {
                HStack(spacing: 10) {
                    ...
                    Button {
                        ...
                        .padding(.vertical, 8)
                        .background(Capsule().fill(AnyShapeStyle(Theme.accentGradient)))
                    ...
                    if model.dnsResults.isEmpty {
                        ...
                        ForEach(model.dnsResults) { r in
                            HStack {
                                Text(r.server)
                                    .font(Theme.mono(11))
                                    .foregroundColor(r.server.hasPrefix(L10n.t("dns.system")) ? Theme.cyan : Theme.textPrimary)
                                Spacer()
                                if let ms = r.ms {
                                    Text(Fmt.ms(ms))
